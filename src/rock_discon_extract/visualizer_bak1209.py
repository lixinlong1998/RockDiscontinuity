...
class ResultsVisualizer:
    ...
    def _PlotStereonet(
        ...
        """
        功能简介:
            对单一结果(out_dir, point_path)绘制极射赤平面投影图,
            使用该结果目录下的 <basename>_discontinuitys.csv 中的 Dip,Dipdir列。

            输出文件:
                对每个输出格式 fmt(如 ".png", ".svg"), 生成:
                    <out_dir>/<base_name>_<plot_short_name><fmt>
                例如:
                    Rock_GLS4_part1_localize_0.05m_stereonet.png

        实现思路:
            1) 构造结构面级 CSV 路径:
                   disc_csv_path = out_dir / "<base_name>_discontinuitys.csv";
            2) 使用 pandas 读取 CSV, 提取 "Dip","Dipdir" 列并规范到[0,90)和[0, 360) 区间;
            3) 以给定的分箱宽度(如 10°)构造方位角和倾角划分;
            4) 在极坐标系中绘制结构面的极射赤平面（等面积/等角）投影点:
                 - 0° 在北(N), 角度顺时针增加;
                 - 每 30° 一个角度刻度;
            5) 调用 _SaveFigure 将图像保存为多种格式。
        输入:
