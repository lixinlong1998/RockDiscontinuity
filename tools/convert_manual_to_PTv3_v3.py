...
SIZE = 10.0  # 切块边长 (m)
DEFAULTS = dict(
    input=r"D:\Research\20250313_RockFractureSeg\_cache\Rock_GLS4_part1_localize_0.02m_ManualSegments_merged_label.csv",
    output=r"D:\Research\20250313_RockFractureSeg\Exp_Pointcept\Pointcept\data\RockDisc",
...
def main():
    ...
    for block_idx, split in tqdm(split_map, desc="  处理中", ncols=70):
        ...
        # ── 保存为目录 + 独立 .npy（Pointcept DefaultDataset 格式）──
        # DefaultDataset.get_data() 调用 os.listdir(sample_dir)，
        # 再对目录内每个 <field>.npy 执行 np.load，字段名即文件名去掉扩展名。
        # 正确结构: train/train_00000/coord.npy  feat.npy  segment.npy  instance.npy
        sample_dir = out_root / split / f"{split}_{count:05d}"
        sample_dir.mkdir(parents=True, exist_ok=True)
        np.save(str(sample_dir / "coord.npy"),    sample["coord"])
        np.save(str(sample_dir / "feat.npy"),     sample["feat"])
        np.save(str(sample_dir / "segment.npy"),  sample["segment"])
        np.save(str(sample_dir / "instance.npy"), sample["instance"])
    ...
    print(f"   目录格式  : Pointcept DefaultDataset 直接兼容")
