...
from .base import ClusteringAlgorithm, PlaneClusterInfo
...
class DBFCMCluster(ClusteringAlgorithm):
    """
    功能简介:
    ...
    输入参数:
        ...
        self: Optional[Clusterself]
            聚类参数配置; 若为 None, 则使用 Clusterself 的默认参数。
    ...
    def __init__(
        ...
        输出:
            无直接输出, Clusterself 实例将被 ClusterDiscontinuities 使用.
        """
        super().__init__(name="DBFCM")  # class Clusterself:
    ...
    def _ClusterImpl(self,
        ...
        if not discontinuities:
            self.logger.warning("ClusterDiscontinuities: 输入的结构面列表为空, 不执行聚类.")
            return None

        # ----------------------
        # Step 0: 提取有效结构面与基础数据
        # ----------------------
        with Timer("Step0_ExtractData", self.logger):
            normals, rmses, lengths, valid_indices = self._ExtractDiscontinuityFeatures(discontinuities)
            n_valid = len(valid_indices)
            if n_valid == 0:
                self.logger.warning(
                    "ClusterDiscontinuities: 没有找到包含有效 plane.normal 与 trace_length 的结构面.")
                return None

            # 将法向统一到上半球并归一化
            normals = self._NormalizeNormalsToUpperHemisphere(normals)

        # ----------------------
        # Step 1: 计算质量优先度 q
        # ----------------------
        with Timer("Step1_ComputeQualityQ", self.logger):
            q_values, rmse_norm, length_norm = self._ComputeQualityScores(
                rmses,
                lengths,
                self.rmse_weight,
                self.length_weight
        ...
        # ----------------------
        with Timer("Step2_DBSCAN_Init", self.logger):
            first_layer_mask, cluster_labels_first, cluster_centers, is_manual_seed = self._RunDbscanOnNormals(
                normals,
                q_values)
            ...
            if manual_dip_dirs:
                cluster_centers, is_manual_seed = self._AppendManualSeeds(
                    cluster_centers,
                    is_manual_seed,
                    manual_dip_dirs,
                    self.dbscan_eps_deg)
            ...
            if n_clusters_init == 0:
                self.logger.warning(
                    "ClusterDiscontinuities: DBSCAN + 人工初始簇总数为 0, 将不执行 FCM, 所有结构面标记为噪声.")
                self._MarkAllAsNoise(discontinuities, valid_indices)
                return []

        # ----------------------
        # Step 3: 计算样本综合质量权重 α_i
        # ----------------------
        with Timer("Step3_ComputeSampleWeights", self.logger):
            alpha = self._ComputeSampleWeights(
                q_values=q_values,
                rmse_norm=rmse_norm,
        ...
        # ----------------------
        with Timer("Step4_WeightedFCM", self.logger):
            centers_fcm, membership = self._RunWeightedFcmOnSphere(
                normals=normals,
                alpha=alpha,
                centers_init=cluster_centers)

            centers_fcm, membership, is_manual_seed_final = self._MergeCloseClusters(
                normals=normals,
                alpha=alpha,
                centers=centers_fcm,
        ...
        # ----------------------
        with Timer("Step5_AssignLabelsAndConfidence", self.logger):
            cluster_infos, labels_final, membership_max, cluster_conf = self._AssignLabelsAndConfidence(
                normals=normals,
                alpha=alpha,
                centers=centers_fcm,
                membership=membership,
        ...
        # # ----------------------
        # if self.enable_plot:
        #     with Timer("Step6_PlotClusters", self.logger):
        #         try:
        #             self._PlotClustersOnStereonet(
        #                 normals=normals,
        #                 labels=labels_final,
        #                 centers=centers_fcm
        #             )
        #         except Exception as e:
        #             self.logger.warning(f"PlaneCluster: 绘图失败: {e}")

        return discontinuities, cluster_infos

    # ======================================================================
    # 内部工具函数实现
    # ======================================================================

    def _ExtractDiscontinuityFeatures(self, discontinuities: List[Discontinuity]) -> Tuple[
        ...
        if not normals_list:
            self.logger.warning("PlaneCluster: 未找到包含有效法向的结构面.")
    ...
    def _NormalizeNormalsToUpperHemisphere(self, normals: np.ndarray) -> np.ndarray:
        """
        功能简介:
    ...
    def _ComputeQualityScores(self,
                              rmses: np.ndarray,
                              lengths: np.ndarray,
                              rmse_weight: float,
                              length_weight: float
    ...
    def _AngularDistanceRad(self, v1: np.ndarray, v2: np.ndarray) -> float:
        """
        功能简介:
            计算两个单位向量之间的夹角距离 (弧度).
    ...
    def _RunDbscanOnNormals(self,
                            normals: np.ndarray,
                            q_values: np.ndarray) -> Tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray]:
        ...
        first_count = max(1, int(round(self.first_layer_ratio * N)))
        ...
        # DBSCAN 参数
        eps_rad = math.radians(self.dbscan_eps_deg)
        min_samples = max(3, int(round(self.dbscan_min_samples_ratio * first_count)))
        ...
        if first_count < min_samples:
            # 第一层样本不足以执行 DBSCAN
            self.logger.warning("PlaneCluster: 第一层样本数不足以执行 DBSCAN, 将跳过初始聚类.")
        ...
        for i in range(first_count):
            ...
            for j in range(first_count):
                nj = normals_first[j]
                theta = self._AngularDistanceRad(ni, nj)
                if theta <= eps_rad:
                    nbrs_i.append(j)
        ...
        if not centers:
            self.logger.warning("PlaneCluster: DBSCAN 未找到任何簇(均为噪声).")
            return first_layer_mask, labels_first, np.zeros((0, 3), dtype=np.float64), np.zeros((0,), dtype=bool)

        cluster_centers = np.vstack(centers).astype(np.float64)
        is_manual_seed = np.zeros((cluster_centers.shape[0],), dtype=bool)
        self.logger.info(f"PlaneCluster: DBSCAN 初始得到 {cluster_centers.shape[0]} 个簇中心.")
        return first_layer_mask, labels_first, cluster_centers, is_manual_seed

    def _ConvertDipDirToNormal(self, dip_deg: float, dip_dir_deg: float) -> np.ndarray:
        """
        功能简介:
    ...
    def _AppendManualSeeds(self,
                           cluster_centers: np.ndarray,
                           is_manual_seed: np.ndarray,
                           manual_dip_dirs: List[Tuple[float, float]],
                           eps_deg: float) -> Tuple[np.ndarray, np.ndarray]:
        """
        功能简介:
        ...
        for dip_deg, dip_dir_deg in manual_dip_dirs:
            n_man = self._ConvertDipDirToNormal(dip_deg, dip_dir_deg)
            if centers.shape[0] > 0:
                angles = np.array(
                    [self._AngularDistanceRad(n_man, c) for c in centers],
                    dtype=np.float64
                )
            ...
            if min_angle > eps_rad:
                ...
                self.logger.info(f"PlaneCluster: 追加人工簇中心 dip={dip_deg}, dip_dir={dip_dir_deg}, "
                                 f"与已有中心最小夹角={math.degrees(min_angle):.2f}°")
            else:
                self.logger.info(f"PlaneCluster: 忽略人工簇中心 dip={dip_deg}, dip_dir={dip_dir_deg}, "
                                 f"因与已有中心夹角过小({math.degrees(min_angle):.2f}°)")

        return centers, manual_flags

    def _ComputeSampleWeights(self,
                              q_values: np.ndarray,
                              rmse_norm: np.ndarray,
                              first_layer_mask: np.ndarray) -> np.ndarray:
        """
        功能简介:
        ...
        if n_remain > 0:
            if self.quality_mode.lower() == "piecewise":
                ...
                for i, p in enumerate(p_remain):
                    if p < 0.2:
                        ...
                        s_remain[i] = self.quality_min_weight
                ...
                s = self.quality_min_weight + (1.0 - self.quality_min_weight) * np.power(
                    q_scaled, self.quality_gamma
                )
                # 第一层重新强制为 1.0
                s[first_layer_mask] = 1.0

        # 二级: 针对全体最低质量 low_quality_ratio 部分做 RMSE 调权
        N_low = max(0, int(round(self.low_quality_ratio * N)))
        beta = np.ones((N,), dtype=np.float64)
        if N_low > 0:
            order_asc = np.argsort(q_values)  # q 从小到大
            low_indices = order_asc[:N_low]
            beta_low = np.exp(-self.rmse_gamma * rmse_norm[low_indices])
            beta[low_indices] = beta_low

        alpha = s * beta
        return alpha

    def _RunWeightedFcmOnSphere(self,
                                normals: np.ndarray,
                                alpha: np.ndarray,
                                centers_init: np.ndarray) -> Tuple[np.ndarray, np.ndarray]:
        """
        功能简介:
        ...
        tol_rad = math.radians(self.fcm_tol_deg)
        eps = 1e-12
        ...
        for it in range(max_iter):
            ...
            for i in range(N):
                for k in range(K):
                    d_ik = self._AngularDistanceRad(normals[i], centers[k])
                    # 避免完全 0 的情况影响换算
                    d[i, k] = max(d_ik, eps)
            ...
            for k in range(K):
                ...
                if w_sum < eps:
                    # 若该簇几乎没有支持样本, 则不更新中心
                    self.logger.warning(f"PlaneCluster: 第 {it} 次迭代时簇 {k} 权重和过小, 保持原中心不变.")
                    continue
                vec_sum = np.sum(normals * weights[:, None], axis=0)
                norm_vec = float(np.linalg.norm(vec_sum))
                if norm_vec < eps:
                    self.logger.warning(f"PlaneCluster: 第 {it} 次迭代时簇 {k} 更新中心向量模长过小, 保持原中心不变.")
                    continue
            ...
            max_angle = 0.0
            for k in range(K):
                angle_k = self._AngularDistanceRad(centers_old[k], centers[k])
                if angle_k > max_angle:
                    max_angle = angle_k

            if max_angle < tol_rad:
                self.logger.info(
                    f"PlaneCluster: FCM 在第 {it} 次迭代时收敛, 最大中心变化角度={math.degrees(max_angle):.4f}°")
                break

        return centers, membership

    def _MergeCloseClusters(self,
                            normals: np.ndarray,
                            alpha: np.ndarray,
                            centers: np.ndarray,
                            membership: np.ndarray,
                            is_manual_seed: np.ndarray) -> Tuple[np.ndarray, np.ndarray, np.ndarray]:
        """
        功能简介:
        ...
        eps_rad = math.radians(self.merge_angle_eps_deg)
        m = self.fcm_m
        eps = 1e-12

        for loop in range(self.max_merge_loops):
            ...
            for p in range(K):
                for q in range(p + 1, K):
                    angle = self._AngularDistanceRad(centers_cur[p], centers_cur[q])
                    angle_mat[p, q] = angle
                    angle_mat[q, p] = angle
            ...
            if min_angle >= eps_rad:
                self.logger.info(f"PlaneCluster: 簇合并停止, 最小中心夹角={math.degrees(min_angle):.3f}° >= 阈值.")
                break
            ...
            self.logger.info(f"PlaneCluster: 合并簇 {p} 与簇 {q}, 中心夹角={math.degrees(min_angle):.3f}°")
    ...
    def _AssignLabelsAndConfidence(self,
                                   normals: np.ndarray,
                                   alpha: np.ndarray,
                                   centers: np.ndarray,
                                   membership: np.ndarray,
                                   is_manual_seed: np.ndarray) -> Tuple[
        List[PlaneClusterInfo], np.ndarray, np.ndarray, Dict[int, float]]:
        """
        功能简介:
        ...
        for i in range(N):
            ...
            if mu_max < self.noise_membership_threshold:
                labels[i] = -1
            else:
                labels[i] = k_best
        ...
        self.logger.info("PlaneCluster: 聚类完成, 有效簇数=%d, 噪声样本数=%d",
                         len(cluster_infos), int(np.sum(labels < 0)))
        return cluster_infos, labels, membership_max, cluster_conf

    def _MarkAllAsNoise(self, discontinuities: List[Discontinuity], valid_indices: List[int]) -> None:
        """
        功能简介:
    ...
    def _PlotClustersOnStereonet(self,
                                 normals: np.ndarray,
                                 labels: np.ndarray,
        ...
        实现思路:
            ...
            注意:
                - 本函数仅在 self.enable_plot=True 且 matplotlib 可用时调用;
