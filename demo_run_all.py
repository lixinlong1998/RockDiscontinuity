...
detector_algorithms = [
    ...
    #     min_inliers=100,
    #     max_iterations=1000,
    #     impl_mode="manual"  # "sklearn" "manual"
    # ),
    #
    # # Open3D 后端 RANSAC
    # RansacDetector(
    ...
    #     min_inliers=100,
    #     max_iterations=1000,
    #     impl_mode="sklearn"  # "sklearn" "manual"
    # ),
    ...
    SupervoxelDetector(
        ...
        ransac_distance=0.22,  # 需要足够精确,但需要顾及测量误差
        ransac_angle=30,  # 不用太过精确，因为实际结构面并不是完美的平面，而是在其内部存在起伏
        min_plane_points=60,  # RANSAC后的内点数只要大于此值则认为是成立的,应该取决于voxel内的平均点数
        edge_distance=0.4,  # edge拼接,需要和平面估计同样严格
        edge_angle=30,  # edge拼接,需要和平面估计同样严格
        min_edge_points=30,  # 启动edge patch detect的最小点数
        min_edge_patch_points=15,  # edge patch detect后的内点数
        super_distance=0.4,  # 吸纳周围散点的初始阈值
        super_angle=40,  # 吸纳周围散点的初始阈值
        distance_step=0.01,  # 2个step保持同步
        angle_step=1.0,  # 2个step保持同步
        max_refit_error=8.0,  # 吸纳周围散点时至多可接受的误差
        patch_distance=0.5,
        patch_angle=30,
        voxel_patch_thickness=0.3,  # 新增：厚度带半厚度
        voxel_patch_bitmap_B=0.05,  # 新增：bitmap 分辨率 B
    ),
    # TODO: 其它检测算法同样在此添加
]
