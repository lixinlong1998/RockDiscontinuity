...
class SupervoxelDetector(PlaneDetectionAlgorithm):
    ...
    def _DetectImpl(self, point_cloud: PointCloud) -> List[Discontinuity]:
        ...
        with Timer("Supervoxel: Voxelization", self.logger):
            ...
            for i, vid in enumerate(map(tuple, voxel_indices)):
                ...
                if vid in voxel_map:
                    ...
                    voxel_map[vid] = [i]
            # 计算平均点数
            avg_voxel_points = np.array([len(voxel_indices) for vid, voxel_indices in voxel_map.items()],
                                        dtype=int).mean()
        self.logger.info(f"Voxelization: generated {len(voxel_map)} voxels with size {self.voxel_size:.3f}m.")
        self.logger.info(f"Voxelization: average of points in voxels is {avg_voxel_points}.")

        # Step 2: 局部共面性检测 (体素内 RANSAC 拟合平面)
        grow_units = {}  # 用于保存所有初始平面生长单元 (grow_units)
            ...
            for voxel_id, pts_indices in voxel_map.items():
                ...
                if len(inlier_set) < self.min_plane_points:
                    ...
                    grow_units[voxel_id] = {
                        "voxel_ids": [voxel_id],
                        "points": inlier_set,
                        "orig_points": inlier_set,  # 原始种子平面点集
                        "normal": (a, b, c),
                        "d": d
                    }
                    # 更新剩余点
        ...
        self.logger.info(
            f"Initial plane extraction: {len(grow_units)} plane seeds found, remaining points in {sum(len(s) for s in remain_points.values())} points.")

        # Step 3: 边界信息计算 (边缘平面提取与合并)
        with Timer("Supervoxel: Edge patch extraction", self.logger):
            # 遍历每个体素的剩余点集
            edge_patches_number = 0
                ...
                while len(pts_set) > self.min_edge_points and testable:
                    ...
                    if len(inlier_set) > self.min_edge_patch_points:
                        ...
                        for neighbor_vid in neighbor_voxels:
                            # 查找该邻域体素对应的生长单元(平面)
                            grow_unit = grow_units[neighbor_vid]
                            # 取该平面参数用于相似性计算
                            plane_normal = grow_unit["normal"]
                            plane_d = grow_unit["d"]
                            ...
                            if mw < self.edge_distance and nw < self.edge_angle:
                                ...
                                if Pw < best_sim:
                                    best_sim = Pw
                                    best_cluster_idx = neighbor_vid
                        if best_cluster_idx is not None:
                            # 将该边缘patch合并到相邻最佳平面生长单元中:contentReference[oaicite:37]{index=37}:contentReference[oaicite:38]{index=38}
                            grow_units[best_cluster_idx]["points"].update(inlier_set)
                            # 记录该邻域体素也包含此patch所在体素
                            if voxel_id not in grow_units[best_cluster_idx]["voxel_ids"]:
                                grow_units[best_cluster_idx]["voxel_ids"].append(voxel_id)
        ...
        self.logger.info(
            f"Edge patch extraction: added {edge_patches_number} edge patches, total planes now {len(grow_units)}.")

        # Step 4: 超体素分割 (以每个平面生长单元为种子扩张)
        supervoxels = []  # 存储超体素平面片段结果
        with Timer("Supervoxel: Supervoxel segmentation", self.logger):
            for voxel_id, cluster in grow_units.items():
                # 每个生长单元作为初始种子
                # 记录原始平面的法向用于误差评估
                ...
                for sv in seed_voxels:
                    neighbor_voxels.append(sv)  # 包含自身体素
                ...
                with Timer(f"Supervoxel growth for seed {voxel_id}", self.logger):
