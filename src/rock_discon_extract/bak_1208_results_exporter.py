...
class ResultsExporter:
    ...
    功能简介:
        ...
        包括:
            1) 点级 CSV (每行一个点, 含平面/结构面信息以及局部曲率、点到平面距离、
               以及基于结构面 ID 生成的统一颜色 DR,DG,DB)
            2) 结构面级 CSV (每行一个 Discontinuity)
            3) 所有结构面的凸包多边形 PLY 文件(便于 MeshLab / CloudCompare 可视化,
               当前实现为 vertex + face, 同时可选 edge)
            4) 额外: 为每个结构面单独导出的点 CSV 与 polygon PLY, 统一存放在
               {base_name}_discontinuitys 子文件夹中, 便于逐个结构面检查。
    ...
    实现思路(概要):
        ...
        - 单独结构面输出:
            在 {base_name}_discontinuitys/ 下, 为每个 Discontinuity 导出:
    ...
    def ExportAll(
        ...
        功能简介:
            ...
            同时, 为每个 Discontinuity 单独导出点 CSV 与 polygon PLY,
            存放于 {basename}_discontinuitys 子目录中。
        ...
        实现思路:
            ...
            5) 在 {basename}_discontinuitys 子目录中, 调用 ExportPerDiscontinuityFiles
               为每个结构面导出单独结果;
            6) 返回包含输出路径信息的字典, 便于上层记录或打印.
        ...
        输出:
            paths: Dict[str, str]
                包含 "dir", "points_csv", "disc_csv", "polygons_ply", "point_cloud_path".
        """
        ...
        # 为每个结构面导出单独的点 CSV 和 polygon PLY
        discon_dir = os.path.join(out_dir, f"{base_name}_discontinuitys")
        self.ExportPerDiscontinuityFiles(discon_dir)
    ...
    def ExportPointLevelCsv(self, csv_path: str) -> None:
        ...
        功能简介:
            ...
            以及基于 Discontinuity_id 生成的统一颜色 DR,DG,DB。
        ...
        with Timer(f"ExportPointLevelCsv({os.path.basename(csv_path)})", self.logger):
            ...
            with open(csv_path, "w", newline="", encoding="utf-8") as f:
                ...
                for disc_id, disc in enumerate(self.discontinuities):
                    ...
                    # 结构面统一颜色 (DR, DG, DB)
                    base_color_r, base_color_g, base_color_b = self._GenerateColorFromId(disc_id)
    ...
    def ExportDiscontinuityLevelCsv(self, csv_path: str) -> None:
        ...
        实现思路:
            对每个 Discontinuity:
                ...
                3) 基于 plane_ref 和 Discontinuity.ComputeGeometry 计算:
                   - Area: 凸包多边形面积;
        ...
        with Timer(
            ...
            with open(csv_path, "w", newline="", encoding="utf-8") as f:
                ...
                for disc_id, disc in enumerate(self.discontinuities):
                    ...
                    disc.ComputeGeometry(self.point_cloud, plane_ref)
    ...
    def ExportDiscontinuityPolygonsToPly(self, ply_path: str) -> None:
        ...
        功能简介:
            ...
            每个结构面的边界以点 + 三角面(face) 的形式表示, 并可选增加边(edge),
            便于在 MeshLab / CloudCompare 中查看多边形边界与面片。

        实现思路:
            对每个 Discontinuity:
                1) 调用 disc.ComputeGeometry(self.point_cloud) 保证 polygon_points 可用;
                2) 从 disc.polygon_points 追加到全局顶点列表;
                3) 使用 _GenerateColorFromId(disc_id) 为该结构面生成统一颜色, 填充到颜色数组;
                4) 为该结构面构造三角面片:
                   - 简单使用“扇形剖分”: (0,i,i+1) 形式;
                5) 可选构造边: 将顶点按顺序首尾相连;
        ...
        with Timer(f"ExportDiscontinuityPolygonsToPly({os.path.basename(ply_path)})", self.logger):
            ...
            for disc_id, disc in enumerate(self.discontinuities):
                ...
                # 统一颜色: 基于 disc_id 生成
                base_r, base_g, base_b = self._GenerateColorFromId(disc_id)
                colors_disc = np.tile(
                    np.array([base_r, base_g, base_b], dtype=np.uint8),
                    (m, 1)
                )
                colors_list.append(colors_disc)

                # 构造三角面: 扇形剖分 (offset, offset+i, offset+i+1)
    ...
    # =========================
    # 每个结构面单独导出: 点 CSV + polygon PLY
    # =========================

    def ExportPerDiscontinuityFiles(self, discon_dir: str) -> None:
        """
        功能简介:
        ...
        """
        discon_dir = os.path.abspath(discon_dir)
        os.makedirs(discon_dir, exist_ok=True)

        with Timer(f"ExportPerDiscontinuityFiles({os.path.basename(discon_dir)})", self.logger):
            num_disc = 0
            num_point_files = 0
            num_poly_files = 0
            ...
            for disc_id, disc in enumerate(self.discontinuities):
                ...
                # 统计 PointsNumber: 优先使用 disc.point_indices
                if getattr(disc, "point_indices", None):
                    unique_indices = set(int(idx) for idx in disc.point_indices)
                else:
                    all_indices: List[int] = []
                    for seg in disc.segments:
                        all_indices.extend(int(i) for i in seg.point_indices)
                    unique_indices = set(all_indices)

                points_number = len(unique_indices)

                # 文件名
                points_csv_path = os.path.join(
                    discon_dir,
                    f"DisconPoints_{disc_id}_{points_number}.csv",
                )
                polygon_ply_path = os.path.join(
                    discon_dir,
                    f"DisconPolygon_{disc_id}_{points_number}.ply",
                )

                # 统一颜色
                base_color_r, base_color_g, base_color_b = self._GenerateColorFromId(disc_id)

                # --------- 导出点 CSV (结构与 ExportPointLevelCsv 一致, 仅当前 disc) ---------
                ...
                # --------- 导出 polygon PLY (仅当前 disc) ---------
                disc.ComputeGeometry(self.point_cloud)
                pts = getattr(disc, "polygon_points", None)
                if pts is None or len(pts) < 3:
                    # 没有有效多边形, 跳过 polygon 导出
                    continue
                ...
                # 统一颜色
                colors_disc = np.tile(
                    np.array([base_color_r, base_color_g, base_color_b], dtype=np.uint8),
                    (m, 1)
                )
                ...
                if m >= 3:
                    for i in range(1, m - 1):
                        faces_disc.append([0, i, i + 1])

                # edges: 首尾相连
                edges_disc: List[List[int]] = []
                if m >= 2:
                    for i in range(m - 1):
                        edges_disc.append([i, i + 1])
                    edges_disc.append([m - 1, 0])

                self._ExportToMeshlabPly(
                    filename=polygon_ply_path,
                    vertices=pts,
                    edges=np.asarray(edges_disc, dtype=np.int32) if edges_disc else None,
                    faces=np.asarray(faces_disc, dtype=np.int32) if faces_disc else None,
                    colors=colors_disc,
                )
            ...
            self.logger.info(
                f"按结构面单独导出完成: {discon_dir}, "
                f"共处理 {num_disc} 个结构面, "
                f"导出 {num_point_files} 个点 CSV, "
                f"{num_poly_files} 个 polygon PLY。"
            )
    ...
    def _CreateOutputSubdir(self, result_root_dir: str, base_name: str) -> str:
        ...
        功能简介:
            ...
            例如: <result_root_dir>/20251119_224205_RANSAC_open3d_<base_name>
    ...
    def _GenerateColorFromId(self, disc_id: int) -> Tuple[int, int, int]:
        """
        功能简介:
            根据结构面 ID 生成一个稳定的伪随机 RGB 颜色, 便于在可视化软件中区分
            不同的 Discontinuity, 同一 ID 在不同运行中颜色保持一致。

        实现思路:
            - 使用一个固定的随机种子基准, 加上 disc_id 作为偏移, 构造 numpy RandomState;
            - 在 [0, 255] 范围内生成 3 个整数作为 (R, G, B);
            - 若需要避免过暗颜色, 可以简单加上一些偏移或拉伸到 [64, 255] 区间,
              当前先保持 [0,255] 全区间分布。
        ...
        输出:
            (r, g, b): Tuple[int, int, int]
                对应的颜色值, 每个分量范围为 [0, 255].
        """
        base_seed = 12345
        rng = np.random.RandomState(base_seed + int(disc_id))
